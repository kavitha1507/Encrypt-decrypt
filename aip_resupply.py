...
from BackupScripts.aip import aipBackup
...
def aip(JID,AID):
	...
	dbCursor.execute('''SELECT AAW.role_id,AAW.is_completed,A.ArticleKey,D.DatasetStatusID,D.DatasetID
		FROM pcv3_aip.Journals J
		JOIN pcv3_aip.Article A ON J.JournalKey=A.JournalKey
		JOIN pcv3_karger_meta.DatasetStatus D ON A.DatasetStatusID=D.DatasetStatusID
		JOIN pcv3_aip.article_actor_workflow AAW ON A.ArticleKey=AAW.article_key
...
def templateToMail(record,JID,AID):
	...
	templateFilePath = os.path.realpath(os.path.join(os.path.dirname(__file__),"Templates/{}".format(template['aip']))) # Change template filename mentioned in configs file
	with open(templateFilePath) as templateFile:
		SQL_DUMP = Template(templateFile.read()).substitute(querydata)
	
	htmldata = data['aip'] # Change data name for mail mentioned in configs file
	...
	SendMail(HTML_FORMAT,JID,AID,toId='raman.gopal@tnqsoftware.co.in')