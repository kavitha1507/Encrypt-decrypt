...
from BackupScripts.rec import recBackup
...
def rec(JID,AID):
	...
	dbCursor.execute('''SELECT AAW.role_id,AAW.is_completed,A.ArticleKey,D.DatasetStatusID,D.DatasetID
		FROM pcv3_rec.Journals J
		JOIN pcv3_rec.Article A ON J.JournalKey=A.JournalKey
		JOIN pcv3_karger_meta.DatasetStatus D ON A.DatasetStatusID=D.DatasetStatusID
		JOIN pcv3_rec.article_actor_workflow AAW ON A.ArticleKey=AAW.article_key
...
def templateToMail(record,JID,AID):
	...
	templateFilePath = os.path.realpath(os.path.join(os.path.dirname(__file__),"Templates/{}".format(template['rec']))) # Change template filename mentioned in configs file
	with open(templateFilePath) as templateFile:
		SQL_DUMP = Template(templateFile.read()).substitute(querydata)
	
	htmldata = data['rec'] # Change data name for mail mentioned in configs file
