...
from BackupScripts.rsc3 import rsc3Backup
...
def rsc3(JID,AID):
	...
	dbCursor.execute('''SELECT AAW.role_id,AAW.is_completed,A.ArticleKey,D.DatasetStatusID,D.DatasetID
		FROM pcv3_rsc.Journals J
		JOIN pcv3_rsc.Article A ON J.JournalKey=A.JournalKey
		JOIN pcv3_rsc_meta.DatasetStatus D ON A.DatasetStatusID=D.DatasetStatusID
		JOIN pcv3_rsc.article_actor_workflow AAW ON A.ArticleKey=AAW.article_key
...
def templateToMail(record,JID,AID):
	...
	templateFilePath = os.path.realpath(os.path.join(os.path.dirname(__file__),"Templates/{}".format(template['rsc3']))) # Change template filename mentioned in configs file
	with open(templateFilePath) as templateFile:
		SQL_DUMP = Template(templateFile.read()).substitute(querydata)
	
	htmldata = data['rsc3'] # Change data name for mail mentioned in configs file
